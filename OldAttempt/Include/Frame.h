...
public:
    ...
    Frame(shared_ptr<ImageData>Img, int id, float ab_exposure, shared_ptr<CalibData>_Calib,
          shared_ptr<photoUndistorter> PhoUndist, bool ForInit = false);
    ~Frame();
    void CreateDirPyrs(vector<float>& Img, vector<Vec3f*> &DirPyr);
    void ReduceToEssential();
    bool PosInGrid(const cv::KeyPoint &kp, int &posX, int &posY);
    void Extract(shared_ptr<FeatureDetector>_Detector, int id ,bool ForInit, shared_ptr<IndexThreadReduce<Vec10>> ThreadPool);
    vector<size_t> GetFeaturesInArea(const float &x, const float  &y, const float  &r) const;
    void ComputeBoVW();

    vector<Vec3f*> DirPyr; //float representation of image pyramid with computation of dIx ad dIy
    vector<float*> absSquaredGrad;
    cv::Mat Image;
    vector<vector<vector<unsigned short int>>> mGrid;
    vector<cv::KeyPoint> mvKeys;
    vector<FrameFramePrecalc, Eigen::aligned_allocator<FrameFramePrecalc>> targetPrecalc;

    vector<shared_ptr<MapPoint>> pointHessians;				// contains all ACTIVE points.
	vector<shared_ptr<MapPoint>> pointHessiansMarginalized;	// contains all MARGINALIZED points (= fully marginalized, usually because point went OOB.)
	vector<shared_ptr<MapPoint>> pointHessiansOut;		// contains all OUTLIER points (= discarded.).
    vector<shared_ptr<ImmaturePoint>> ImmaturePoints;
...
struct FrameShell
{
    ...
    inline FrameShell(shared_ptr<ImageData> Img, shared_ptr<CalibData>_Calib,
                      shared_ptr<photoUndistorter> _phoUndist, bool ForInit)
    {
        ...
        ab_exposure = Img->Exposure; //Exposure time of the left image
        poseValid = false;
        isKeyFrame = false;
        frame = shared_ptr<Frame> (new Frame(Img, id, ab_exposure, _Calib, _phoUndist ,ForInit ));
    }
