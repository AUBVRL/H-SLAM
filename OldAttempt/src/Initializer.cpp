...
Initializer::Initializer(shared_ptr<CalibData> _Calib, shared_ptr<IndexThreadReduce<Vec10>> FrontEndThreadPoolLeft) : //, shared_ptr<GUI> _DisplayHandler
                                            thPool(FrontEndThreadPoolLeft), Calib(_Calib) //, displayhandler(_DisplayHandler)
{
    ...
    randomGen = shared_ptr<cv::RNG>(new cv::RNG(0));
}

bool Initializer::Initialize(shared_ptr<FrameShell> _Frame)
{
    ...
    if (NumFails > 40)
    {
        ...
        if (!FirstFrame)
        {
            ...
            {
                FirstFrame = _Frame;
                FirstFrame->frame->Image.copyTo(TransitImage);
        ...
        {
            NumFails = 0;
            nmatches = count(vbTriangulated.begin(), vbTriangulated.end(), true);

            if (nmatches < 0.1 * FirstFrame->frame->nFeatures || nmatches < 200) //did not triangulate enough repeat the process! 
            {
                FirstFrame->frame.reset();
                FirstFrame.reset();
                return false;
            ...
            //calc opt flow with new priors here
            ...
            //Given pose -> epipolar geometry -> trace for new matches -> triangulate new matches -> refine optimization


            // //Direct Refinement statistics!!
            // vector<float> pts;
            // // vector<float> RepErrorAft;
            ...
            //     }
            // }
            // // // cout<<"Mean Rep Error Befor: "<< accumulate(RepErrorBef.begin(), RepErrorBef.end(), 0.0)/RepErrorBef.size();
            // // // cout<<"  Mean Rep Error After: "<<accumulate(RepErrorAft.begin(), RepErrorAft.end(), 0.0)/RepErrorAft.size() <<endl;
...
{
    Calib.reset();
    mvSets.clear(); mvSets.shrink_to_fit();
    TransitImage.release();
    FirstFramePts.clear(); FirstFramePts.shrink_to_fit();
    mvbPrevMatched.clear(); mvbPrevMatched.shrink_to_fit();
    videpth.clear(); videpth.shrink_to_fit();
    MatchedPts.clear(); MatchedPts.shrink_to_fit();
    MatchedPtsBkp.clear(); MatchedPtsBkp.shrink_to_fit();
    MatchedStatus.clear(); MatchedStatus.shrink_to_fit();
    ColorVec.clear(); ColorVec.shrink_to_fit();
    mvIniP3D.clear(); mvIniP3D.shrink_to_fit();
    thPool.reset();
    FirstFrame.reset();
    SecondFrame.reset();
}

// int Initializer::MatchForTriangulation(SE3 Pose, shared_ptr<FrameShell>firstFrame, shared_ptr<FrameShell>SecondFrame, vector<bool> &Triangulated)
// {
//     int TH_LOW = 50;
//     int TH_HIGH = 100;
//     bool mbCheckOrientation = true;

//     for (int i = 0, iend = Triangulated.size(); i < iend; ++i)
//     {
//         if(Triangulated[i])
//             continue;
//     }
//     const DBoW3::FeatureVector &vFeatVec1 = firstFrame->frame->mFeatVec;
//     const DBoW3::FeatureVector &vFeatVec2 = SecondFrame->frame->mFeatVec;
...
int Initializer::FindMatches(int windowSize, int maxL1Error)
{
    ...
    calcOpticalFlowPyrLK(TransitImage, SecondFrame->frame->Image, mvbPrevMatched, MatchedPts, status, err, Size(windowSize, windowSize), 2, criteria, flowFlag);
    ...
    SecondFrame->frame->Image.copyTo(TransitImage);
    ...
    if (ShowInitializationMatches)
    {
        ...
        if(ShowInitializationMatchesSideBySide)
            hconcat(FirstFrame->frame->Image, SecondFrame->frame->Image, Image);
        else 
            SecondFrame->frame->Image.copyTo(Image);

        cvtColor(Image, Image, CV_GRAY2RGB);
        for (int i = 0; i < FirstFramePts.size(); ++i)
            if (MatchedStatus[i])
                line(Image, FirstFramePts[i], cv::Point2f(ShowInitializationMatchesSideBySide ? FirstFrame->frame->Image.cols : 0, 0) +  MatchedPts[i], ColorVec[i], 1);
                                ...
                                const cv::Mat &K, vector<cv::Point3f> &vP3D, float th2, vector<bool> &vbGood, float &parallax)
{
    ...
    shared_ptr<CheckRTIn> ChRT = shared_ptr<CheckRTIn>(new CheckRTIn());
    ...
    ChRT->vKeys1 = &vKeys1; ChRT->vKeys2 = &vKeys2; ChRT->vP3D = &vP3D; ChRT->thPoolLock = shared_ptr<mutex>(new mutex);

    ChRT->nGood = 0;

    thPool->reduce( bind( &Initializer::ParallelCheckRT, this, ChRT ,placeholders::_1, placeholders::_2), 0, vbMatchesInliers.size(), ceil(vbMatchesInliers.size() / NUM_THREADS));
...
void Initializer::ParallelCheckRT(shared_ptr<CheckRTIn> In, int min, int max)
{
    ...
    for (int i = min; i < max; ++i)
    {
        ...
        {
            unique_lock<mutex> lock(*In->thPoolLock);
...
float Initializer::ComputeSceneMedianDepth(const int q, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
...
float Initializer::ComputeSceneMeanDepth(vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    vector<float> vDepths;
    vDepths.reserve(FirstFrame->frame->nFeatures);
...
DirectRefinement::DirectRefinement(shared_ptr<CalibData> _Calib, vector<cv::Point3f> &_Pts3D, vector<bool> &_Triangulated, shared_ptr<FrameShell> _FirstFrame,
	...
	applyStep(0);
		...
		if(printDebug)
		{
			printf("lvl %d, it %d (l=%f) %s: %.3f+%.5f -> %.3f+%.5f (%.3f->%.3f) (|inc| = %f)! \t",
					...
					(resOld[0]+resOld[1]) / resOld[2],
					0.0f);
			cout << refToNew_current.log().transpose() << " AFF " << refToNew_aff_current.vec().transpose() <<"\n";
		}
		...
		while(true)
		{
			...
			if(printDebug)
			{
				printf("lvl %d, it %d (l=%f) %s: %.5f + %.5f + %.5f -> %.5f + %.5f + %.5f (%.2f->%.2f) (|inc| = %f)! \t",
						...
						eTotalNew / resNew[2],
						inc.norm());
				cout << refToNew_new.log().transpose() << " AFF " << refToNew_aff_new.vec().transpose() <<"\n";
			}
...
void DirectRefinement::trace(Pnt* _pl)
{
    ...
    {
        cv::hconcat(FirstFrame->frame->Image, SecondFrame->frame->Image, Image);
        cvtColor(Image, Image, CV_GRAY2RGB);
    ...
    for (int i = 0; i < FirstFrame->frame->nFeatures; ++i)
    {
        ...
        float v = FirstFrame->frame->mvKeys[i].pt.y;
        ...
        if (idepth_min > idepth_max)
            swap<float>(idepth_min, idepth_max);

        if (!isfinite(idepth_min) || !isfinite(idepth_max) || (idepth_max < 0))
        {
...
Vec3f DirectRefinement::calcResAndGS( int lvl, Mat88f &H_out, Vec8f &b_out, Mat88f &H_out_sc, Vec8f &b_out_sc, const SE3 &refToNew, AffLight refToNew_aff, bool plot)
{
	...
	float fxl = Calib->pyrK[lvl](0,0); //  pyrfx[lvl];
	float fyl = Calib->pyrK[lvl](1,1); //fy
	float cxl = Calib->pyrK[lvl](0,2);
	float cyl = Calib->pyrK[lvl](1,2);
	...
	for(int i=0;i<npts;i++)
	{
		...
		for(int idx=0;idx<patternNum;idx++)
		{
			...
			if(!isfinite(rlR) || !isfinite((float)hitColor[0]))
			{
				isGood = false;
				break;
			}
...
void DirectRefinement::applyStep(int lvl)
{
	...
	swap<Vec10f*>(JbBuffer, JbBuffer_new);
}
...
void DirectRefinement::optReg(int lvl)
{
	...
	for(int i=0;i<npts;i++)
	{
		...
		// if(nnn > 2)
		// {
		// 	nth_element(idnn,idnn+nnn/2,idnn+nnn);
...
void DirectRefinement::debugPlot(Pnt* Points)
{
    ...
    cv::Mat Depth; 
    cv::cvtColor(FirstFrame->frame->Image, Depth, CV_GRAY2RGB);
	...
	for(int i=0;i<npts;i++)
	{
        ...
        setPixel9(Depth, floor(Points[i].v + 0.5f), floor(Points[i].u + 0.5f), Color);
    }
