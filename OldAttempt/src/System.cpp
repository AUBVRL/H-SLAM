...
namespace SLAM
{
    ...
    void System::ProcessFrame(shared_ptr<ImageData> dataIn)
    {
        ...
        else
        {
            ...
            vector<shared_ptr<MapPoint>> allMps;
            getAllFrameMPs(coarseTracker->lastRef, allMps);
            cout << "allMps size: " << allMps.size() << endl;
    ...
    bool System::TrackIndirect(shared_ptr<FrameShell> currentFrame, SE3 MotionModel)
    {
        bool bOK = false;
    ...
    void System::getAllFrameMPs (shared_ptr<FrameShell> _In, vector<shared_ptr<MapPoint>>& _Out)
    {
        _Out.clear();
        if (!_In->frame)
            return;
        
        shared_ptr<Frame> frame = _In->frame;

        _Out.reserve(frame->nFeatures);

        for (int i = 0, sz = frame->pointHessians.size(); i < sz; ++i)
        {
            cout << "here" << endl;
            if(frame->pointHessians[i])
            {
                PtStatus curPtStat = frame->pointHessians[i]->getPointStatus();
                if (curPtStat == PtStatus::ACTIVE || curPtStat == PtStatus::MARGINALIZED)
                    _Out.push_back(frame->pointHessians[i]);
            }
        }

        for (int i = 0, sz = frame->pointHessiansMarginalized.size(); i < sz; ++i)
        {
            if(frame->pointHessiansMarginalized[i])
            {
                PtStatus curPtStat = frame->pointHessiansMarginalized[i]->getPointStatus();
                if (curPtStat == PtStatus::ACTIVE || curPtStat == PtStatus::MARGINALIZED)
                    _Out.push_back(frame->pointHessiansMarginalized[i]);
            }
        }
    }
