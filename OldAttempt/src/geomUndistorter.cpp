...
geomUndistorter::geomUndistorter(std::string GeomCalib_)
{
    LoadGeometricCalibration(GeomCalib_);
}

geomUndistorter::~geomUndistorter()
{
    if (remapX != 0)    delete[] remapX;
    if (remapY != 0)    delete[] remapY;
}

void geomUndistorter::LoadGeometricCalibration(std::string GeomCalibPath)
{
    passthrough=false;
	remapX = 0;
	remapY = 0;
    ...
    if(!CalibIn.isOpened())
    {cout<<"could not read Geometric calibration data from: "<< GeomCalibPath<<endl; exit(1);}
    ...
    CalibIn["Camera.K"] >> K_l;

    std::string Calibmodel = CalibIn["Calibration.model"];
    if(Calibmodel == "RadTan")
    { Cameramodel = CamModel::RadTan; CalibIn["Camera.dist"] >> distM; }
    else if (Calibmodel == "Atan")
    { Cameramodel = CamModel::Atan; atanDist = CalibIn["Camera.dist"];}
    else if (Calibmodel == "Pinhole")
    { Cameramodel = CamModel::Pinhole; atanDist = (double)0.0; }
    else if (Calibmodel == "EquiDistant")
    { Cameramodel = CamModel::EquiDistant; CalibIn["Camera.dist"] >> distM; }
    else if (Calibmodel == "KannalaBrandt")
    { Cameramodel = CamModel::KannalaBrandt; CalibIn["Camera.dist"] >> distM; }
    else {cout<<"Camera calibration model not specified! exit."<<endl; exit(1);};
    ...
    if( (Cameramodel == CamModel::EquiDistant || Cameramodel == CamModel::RadTan || Cameramodel== CamModel::KannalaBrandt) && distM.empty())
        {cout<<"Error reading camera distortion! exit."<<endl; exit(1);}
    ...
    {
        if (w != wOrg || h != hOrg)
            {std::cout<<"ERROR: rectification mode none requires input and output dimenstions to match!"<<std::endl; exit(1);}
    ...
    {
        cv::Mat DesiredK;
        CalibIn["Calib.desiredK"] >> DesiredK;
        if (DesiredK.empty() || DesiredK.at<double>(0, 2) > 1 || DesiredK.at<double>(0, 3) > 1)
            {std::cout<<"Error reading desired camera calibration! it should be fx fy cx cy relative to the width and height exit."<<std::endl; exit(1);}
...
void geomUndistorter::makeOptimalK_crop()
{
	...
	while(oobLeft || oobRight || oobTop || oobBottom)
	{
		...
		if(iteration > 500)
			{std::cout<<"FAILED TO COMPUTE GOOD CAMERA MATRIX - SOMETHING IS SERIOUSLY WRONG. ABORTING"<<std::endl; exit(1);}
	}
...
void geomUndistorter::distortCoordinates(float* in_x, float* in_y, float* out_x, float* out_y, int n)
{
...
void geomUndistorter::undistort(cv::Mat &Img)
{
    if (!passthrough)
