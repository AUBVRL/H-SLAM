...
namespace SLAM
{
	...
	EIGEN_STRONG_INLINE bool projectPoint(const float &u_pt, const float &v_pt, const float &idepth, const Mat33f &KRKi,
										  const Vec3f &Kt, float &Ku, float &Kv, shared_ptr<CalibData> const &HCalib)
	{
	...
	EIGEN_STRONG_INLINE bool projectPoint(const float &u_pt, const float &v_pt, const float &idepth, const int &dx,
										  const int &dy, shared_ptr<CalibData> const &HCalib, const Mat33f &R, const Vec3f &t,
