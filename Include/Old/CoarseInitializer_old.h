...
public:
	...
	void setFirst(	std::shared_ptr<CalibData> HCalib, std::shared_ptr<Frame> newFrameHessian);
	bool trackFrame(std::shared_ptr<Frame> _newFrame, std::shared_ptr<GUI> DisplayHandler);
	void calcTGrads(std::shared_ptr<Frame> _newFrame);

	int frameID;
	bool fixAffine;
	bool printDebug;
	std::vector<std::vector<std::shared_ptr<Pnt>>> points;
	// Pnt* points[DirPyrLevels];
	std::vector<int> numPoints;
	// int numPoints[DirPyrLevels];
...
private:
    ...
    void debugPlot(int lvl, std::shared_ptr<GUI> DisplayHandler);
	void makeNN();
};
