...
PixelSelector::PixelSelector(std::shared_ptr<CalibData>_Calib)
{
    Calib = _Calib;
    area = Calib->Width*Calib->Height;
    int w = Calib->Width;
    int h = Calib->Height;
	randomPattern = new unsigned char[area];
	std::srand(3141592);	// want to be deterministic.
	for(int i=0;i<area;++i) randomPattern[i] = rand() & 0xFF;
...
void PixelSelector::makeHists(std::vector<float*>& GradPyr, int id)
{
	gradHistFrame = id;
	float * mapmax0 = GradPyr[0];
...
//	else
	{
		...
		if(id != gradHistFrame) makeHists(GradPyr, id);

		// select!
		Eigen::Vector3i n = this->select(DirPyr, id, GradPyr, map_out,currentPotential, thFactor);
	...
	//				idealPotential);
			...
			return makeMaps(DirPyr, id, GradPyr, map_out, density, recursionsLeft-1, plot,thFactor);
		}
...
Eigen::Vector3i PixelSelector::select(std::vector<Vec3f*>&DirPyr, int id, std::vector<float*>& GradPyr,
		float* map_out, int pot, float thFactor)
{

	Eigen::Vector3f const * const map0 = DirPyr[0];

	float * mapmax0 = GradPyr[0];
	float * mapmax1 = GradPyr[1];
	float * mapmax2 = GradPyr[2];
