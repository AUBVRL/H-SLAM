...
CoarseTracker::CoarseTracker(shared_ptr<CalibData> _geomCalib) : lastRef_aff_g2l(0,0)
{
	geomCalib = _geomCalib;
	// make coarse tracking templates.
	for(int lvl=0; lvl<pyramidSize; ++lvl)
	{
		int wl = geomCalib->Width >> lvl;
		int hl = geomCalib->Height >> lvl;
	...
	// warped buffers
    buf_warped_idepth = allocAligned<4,float>(geomCalib->mulWH, ptrToDelete);
    buf_warped_u = allocAligned<4,float>(geomCalib->mulWH, ptrToDelete);
    buf_warped_v = allocAligned<4,float>(geomCalib->mulWH, ptrToDelete);
    buf_warped_dx = allocAligned<4,float>(geomCalib->mulWH, ptrToDelete);
    buf_warped_dy = allocAligned<4,float>(geomCalib->mulWH, ptrToDelete);
    buf_warped_residual = allocAligned<4,float>(geomCalib->mulWH, ptrToDelete);
    buf_warped_weight = allocAligned<4,float>(geomCalib->mulWH, ptrToDelete);
    buf_warped_refColor = allocAligned<4,float>(geomCalib->mulWH, ptrToDelete);
...
void CoarseTracker::makeCoarseDepthL0(vector<shared_ptr<FrameShell>>& frameHessians)
{
	// make coarse tracking templates for latstRef.
	memset(idepth[0], 0, sizeof(float) * geomCalib->mulWH);
	memset(weightSums[0], 0, sizeof(float) * geomCalib->mulWH);
	for(auto &fh : frameHessians)
	{
		for(auto &ph : fh->frame->pointHessians)
		{
			...
			if(ph->lastResiduals[0].first != 0 && ph->lastResiduals[0].second == ResState::IN)
			{
				...
				assert(r->isActive() && r->target == lastRef);
				int u = r->centerProjectedTo[0] + 0.5f;
				int v = r->centerProjectedTo[1] + 0.5f;
				float new_idepth = r->centerProjectedTo[2];
				float weight = sqrtf(1e-3 / (ph->efPoint->HdiF+1e-12));

				idepth[0][u+geomCalib->Width*v] += new_idepth *weight;
				weightSums[0][u+geomCalib->Width*v] += weight;
			}
		}
	}

	for(int lvl=1; lvl<pyramidSize; ++lvl)
	{
		int lvlm1 = lvl-1;
		int wl = geomCalib->wpyr[lvl], hl = geomCalib->hpyr[lvl], wlm1 = geomCalib->wpyr[lvlm1];
	...
	for(int lvl=0; lvl<2; lvl++)
	{
		...
		{
			int wh = geomCalib->wpyr[lvl]*geomCalib->hpyr[lvl]-geomCalib->wpyr[lvl];
			int wl = geomCalib->wpyr[lvl];
			float* weightSumsl = weightSums[lvl];
			float* weightSumsl_bak = weightSums_bak[lvl];
			memcpy(weightSumsl_bak, weightSumsl, geomCalib->wpyr[lvl] * geomCalib->hpyr[lvl]*sizeof(float));
			...
			for(int i=geomCalib->wpyr[lvl];i<wh;i++)
			{
	...
	for(int lvl=2; lvl<pyramidSize; ++lvl)
	{
		int wh = geomCalib->wpyr[lvl]*geomCalib->hpyr[lvl]-geomCalib->wpyr[lvl];
		int wl = geomCalib->wpyr[lvl];
		float* weightSumsl = weightSums[lvl];
	...
	for(int lvl=0; lvl<pyramidSize; ++lvl)
	{
		...
		Eigen::Vector3f* dIRefl = lastRef->frame->DirPyr[lvl];

		int wl = geomCalib->wpyr[lvl], hl = geomCalib->hpyr[lvl];
...
void CoarseTracker::calcGSSSE(float fx, float fy, Mat88 &H_out, Vec8 &b_out, const SE3 &refToNew, AffLight aff_g2l)
{
	acc.initialize();

	__m128 fxl = _mm_set1_ps(fx);
	__m128 fyl = _mm_set1_ps(fy);
...
Vec6 CoarseTracker::calcRes(int lvl, const SE3 &refToNew, AffLight aff_g2l, float cutoffTH)
{
	...
	int wl = geomCalib->wpyr[lvl];
	int hl = geomCalib->hpyr[lvl];
	Eigen::Vector3f* dINewl = newFrame->frame->DirPyr[lvl];
	float fxl = (float) geomCalib->pyrK[lvl](0,0);
	float fyl = (float) geomCalib->pyrK[lvl](1,1); //fy[lvl];
	float cxl = (float) geomCalib->pyrK[lvl](0,2); //cx[lvl];
	float cyl = (float) geomCalib->pyrK[lvl](1,2); //cy[lvl];


	Mat33f Ki = geomCalib->pyrKi[lvl].cast<float>();
	Mat33f RKi = (refToNew.rotationMatrix() * geomCalib->pyrKi[lvl]).cast<float>();
	...
	for(int i=0;i<nl;i++)
	{
		...
		if(lvl==0 && i%32==0)
		{
			...
			Vec3f ptT2 = Ki * Vec3f(x, y, 1) - t*id;
...
void CoarseTracker::setCoarseTrackingRef(
		std::vector<std::shared_ptr<FrameShell>>& frameHessians)
{
	assert(frameHessians.size()>0);
	lastRef = frameHessians.back();
	makeCoarseDepthL0(frameHessians);

	refFrameID = lastRef->id;
	lastRef_aff_g2l = lastRef->frame->efFrame->aff_g2l();

	firstCoarseRMSE=-1;

}

bool CoarseTracker::trackNewestCoarse( std::shared_ptr<FrameShell>& newFrameHessian, SE3 &lastToNew_out, AffLight &aff_g2l_out, int coarsestLvl, Vec5 minResForAbort)
{
	// debugPlot = setting_render_displayCoarseTrackingFull;
	debugPrint = false;

	assert(coarsestLvl < 5 && coarsestLvl < pyramidSize);
	...
	for(int lvl=coarsestLvl; lvl>=0; lvl--)
	{
		...
		float fx = (float) geomCalib->pyrK[lvl](0,0); 
		float fy = (float) geomCalib->pyrK[lvl](1,1); 
		calcGSSSE(fx, fy, H, b, refToNew_current, aff_g2l_current);
		...
		for(int iteration=0; iteration < maxIterations[lvl]; iteration++)
		{
			...
			if(accept)
			{
				float fx = (float) geomCalib->pyrK[lvl](0,0); 
				float fy = (float) geomCalib->pyrK[lvl](1,1); 
				calcGSSSE(fx, fy, H, b, refToNew_new, aff_g2l_new);
				resOld = resNew;
...
std::vector<pair<float, int>> CoarseTracker::GetKFDepthMap()
{
	int wl = geomCalib->wpyr[0], hl = geomCalib->hpyr[0];   
	std::vector<pair<float, int>> allID;
	for (int i = 0, iend = wl * hl ; i < iend ; ++i)
	{
		if (idepth[0][i] > 0)
			allID.push_back(make_pair(idepth[0][i],i));
	}
...
CoarseDistanceMap::CoarseDistanceMap(shared_ptr<CalibData> _geomCalib)
{
	geomCalib = _geomCalib;
	fwdWarpedIDDistFinal = new float[geomCalib->mulWH/4];

	bfsList1 = new Eigen::Vector2i[geomCalib->mulWH/4];
	bfsList2 = new Eigen::Vector2i[geomCalib->mulWH/4];

	int fac = 1 << (pyramidSize-1);


	coarseProjectionGrid = new PointFrameResidual*[2048*(geomCalib->mulWH/(fac*fac))];
	coarseProjectionGridNum = new int[geomCalib->mulWH/(fac*fac)];
...
void CoarseDistanceMap::makeDistanceMap( std::vector<std::shared_ptr<FrameShell>>& frameHessians, std::shared_ptr<FrameShell>& frame)
{
	int w1 = geomCalib->wpyr[1];
	int h1 = geomCalib->hpyr[1];
	int wh1 = w1*h1;
	for(int i=0;i<wh1;i++)
	...
	for(auto &fh : frameHessians)
	{
		...
		SE3 fhToNew = frame->frame->efFrame->PRE_worldToCam * fh->frame->efFrame->PRE_camToWorld;
		...
		for(auto &ph : fh->frame->pointHessians)
		{
			...
			if(!(u > 0 && v > 0 && u < geomCalib->wpyr[1] && v < geomCalib->hpyr[1])) continue;
...
{
	assert(geomCalib->Width != 0);
	int w1 = geomCalib->wpyr[1], h1 = geomCalib->hpyr[1];
	for(int k=1;k<40;k++)
	{
		int bfsNum2 = bfsNum;
...
{
	if(geomCalib->wpyr[0] == 0) return;
	bfsList1[0] = Eigen::Vector2i(u,v);
	fwdWarpedIDDistFinal[u+geomCalib->wpyr[1]*v] = 0;
	growDistBFS(1);
}
