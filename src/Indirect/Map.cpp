...
namespace HSLAM
{
    ...
    {
        d_mvInvertedFile.resize(dbow_Vocab.size());
    }
    ...
    void KeyFrameDatabase::add(shared_ptr<Frame> pKF)
    {
        ...
        if(fbow_Vocab.isValid())
            for(auto &w: pKF->f_mBowVec)  
                f_mvInvertedFile[w.first].insert(pKF);
        else if(!dbow_Vocab.empty())
            for (DBoW3::BowVector::const_iterator vit = pKF->d_mBowVec.begin(), vend = pKF->d_mBowVec.end(); vit != vend; vit++)
                d_mvInvertedFile[vit->first].push_back(pKF);
    }
    ...
    void KeyFrameDatabase::erase(shared_ptr<Frame> pKF)
    {
        ...
        if(fbow_Vocab.isValid()){
            for(auto &w : pKF->f_mBowVec)
                f_mvInvertedFile[w.first].erase(pKF);
        }
        else if(!dbow_Vocab.empty()){
            for (DBoW3::BowVector::const_iterator vit = pKF->d_mBowVec.begin(), vend = pKF->d_mBowVec.end(); vit != vend; vit++){
                // List of keyframes that share the word
                list<shared_ptr<Frame>> &lKFs = d_mvInvertedFile[vit->first];
    ...
    {
        f_mvInvertedFile.clear();
        d_mvInvertedFile.clear();
        d_mvInvertedFile.resize(dbow_Vocab.size());
    }

    vector<shared_ptr<Frame>> KeyFrameDatabase::DetectF_LoopCandidates(shared_ptr<Frame> pKF, float minScore)
    {
        ...
        // Discard keyframes connected to the query keyframe
        {
            ...
            for(auto &w : pKF->f_mBowVec){
                if ( !f_mvInvertedFile.count(w.first)) continue;
                const std::set<std::shared_ptr<HSLAM::Frame>> &lKFs=f_mvInvertedFile.at(w.first);
                for (auto &pKFi:lKFs){
        ...
        for (list<shared_ptr<Frame>>::iterator lit = lKFsSharingWords.begin(), lend = lKFsSharingWords.end(); lit != lend; lit++)
        {
            ...
            if (pKFi->mnLoopWords > minCommonWords)
            {
                ...
                float si = fbow::fBow::score(pKF->f_mBowVec, pKFi->f_mBowVec);
                pKFi->mLoopScore = si;
                if (si >= minScore)
    ...
    vector<shared_ptr<Frame>> KeyFrameDatabase::DetectD_LoopCandidates(shared_ptr<Frame> pKF, float minScore)
    {
        set<shared_ptr<Frame>, std::owner_less<std::shared_ptr<Frame>>> spConnectedKeyFrames = pKF->GetConnectedKeyFrames();
        // auto vConnectedKeyFrames = pKF->GetCovisiblesByWeight(30);
        // std::set<shared_ptr<Frame>, std::owner_less<std::shared_ptr<Frame>>> spConnectedKeyFrames(vConnectedKeyFrames.begin(), vConnectedKeyFrames.end()); 
        ...
        for (list<shared_ptr<Frame>>::iterator lit = lKFsSharingWords.begin(), lend = lKFsSharingWords.end(); lit != lend; lit++)
        {
            ...
            if (pKFi->mnLoopWords > minCommonWords)
            {
                ...
                float si =dbow_Vocab.score(pKF->d_mBowVec, pKFi->d_mBowVec); //mpVoc->score
                pKFi->mLoopScore = si;
                if (si >= minScore)
                    lScoreAndMatch.push_back(make_pair(si, pKFi));
            }
