...
{
	points.resize(DirPyrLevels);
	numPoints.resize(DirPyrLevels);
...
void CoarseInitializer::setFirst(std::shared_ptr<CalibData> HCalib, std::shared_ptr<Frame> _newFrame)
{
	// makeK(HCalib);
	firstFrame = _newFrame;

	// PixelSelector sel(w[0],h[0]);

	float *statusMap = new float[HCalib->wpyr[0] * HCalib->hpyr[0]];
	bool* statusMapB = new bool[HCalib->wpyr[0] * HCalib->hpyr[0]];
	...
	for(int lvl=0; lvl<DirPyrLevels; lvl++)
	{
		...
		if(!points[lvl].empty()) 
			points[lvl].clear(); 
		points[lvl].resize(npts);
		
		// set idepth map to initially 1 everywhere.
		int wl = HCalib->wpyr[lvl], hl = HCalib->hpyr[lvl];
		// Pnt* pl = points[lvl];
		int nl = 0;
		...
		for(int x=patternPadding+1;x<wl-patternPadding-2;x++)
		{
			...
			if((lvl!=0 && statusMapB[x+y*wl]) || (lvl==0 && statusMap[x+y*wl] != 0))
			{
				...
				pl->energy.setZero();
				pl->lastHessian=0;
				pl->lastHessian_new=0;
				pl->my_type= (lvl!=0) ? 1 : statusMap[x+y*wl];
				points[lvl][nl] = pl; 
...
bool CoarseInitializer::trackFrame(std::shared_ptr<Frame> _newFrame, std::shared_ptr<GUI> DisplayHandler)
{
	newFrame = _newFrame;
	...
	for(int lvl=DirPyrLevels-1; lvl>=0; lvl--)
	{



		if(lvl<DirPyrLevels-1)
			propagateDown(lvl+1);
	...
	for(int i=0;i<DirPyrLevels-1;i++)
		propagateUp(i);
...
void CoarseInitializer::debugPlot(int lvl, std::shared_ptr<GUI> DisplayHandler)
{
    bool needCall = false;

    needCall = needCall || DisplayHandler->needPushDepthImage();
    if(!needCall) return;
	...
	cv::Mat iRImg = cv::Mat (wl, hl, CV_32f, firstFrame->dIp[lvl]);
	...
	//IOWrap::displayImage("idepth-R", &iRImg, false);
        DisplayHandler->pushDepthImage(&iRImg);
}
...
void CoarseInitializer::resetPoints(int lvl)
{
	...
	for(int i=0;i<npts;i++)
	{
		...
		if(lvl==DirPyrLevels-1 && !pts[i].isGood)
		{
			float snd=0, sn=0;
			for(int n = 0;n<10;n++)
			{
...
// 	for (int level = 1; level < DirPyrLevels; ++ level)
// 	{
// 		w[level] = w[0] >> level;
// 		h[level] = h[0] >> level;
...
// 	for (int level = 0; level < DirPyrLevels; ++ level)
// 	{
